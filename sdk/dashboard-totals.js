...
function render() {
  ...
  try {
    ...
    if (all.length === 0) { console.log(`No data yet.`); return; }
    ...
    const p1m  = 1000000  * (parseFloat(best.spreadPct)/100) - GAS;
    const p5m  = 5000000  * (parseFloat(best.spreadPct)/100) - GAS;
    const p10m = 10000000 * (parseFloat(best.spreadPct)/100) - GAS;
    console.log(`  $1M: ${GREEN}$${p1m.toFixed(0)}${RESET}  |  $5M: ${GREEN}$${p5m.toFixed(0)}${RESET}  |  $10M: ${GREEN}$${p10m.toFixed(0)}${RESET}`);
    console.log('');

    // ─── CAPTURABLE PROFIT ───────────────────────────────────────
    console.log(`${BOLD}TOTAL CAPTURABLE PROFIT (capturable=true events only)${RESET}`);
    console.log(`${DIM}${'─'.repeat(62)}${RESET}`);
    const capturable = all.filter(o => o.capturable === true);
    const notCapturable = all.filter(o => o.capturable !== true);
    console.log(`  Capturable events:       ${BOLD}${GREEN}${capturable.length.toLocaleString()}${RESET}  ${DIM}of ${all.length} total (${(capturable.length/all.length*100).toFixed(1)}%)${RESET}`);
    console.log(`  Non-capturable events:   ${DIM}${notCapturable.length.toLocaleString()}  (${(notCapturable.length/all.length*100).toFixed(1)}%)${RESET}`);
    console.log('');
    console.log(`${DIM}  Capital      Raw capturable   Realistic (46%)   Daily avg${RESET}`);
    console.log(`${DIM}${'─'.repeat(62)}${RESET}`);
    for (const cap of [5000, 10000, 50000, 100000]) {
      const raw = capturable.reduce((sum, o) => {
    ...
    // ─── ACCUMULATED LOSSES ──────────────────────────────────────
    console.log(`${BOLD}ACCUMULATED LOSSES — TRADES THAT WOULD HAVE LOST MONEY${RESET}`);
    console.log(`${DIM}${'─'.repeat(62)}${RESET}`);
    console.log(`${DIM}  A loss occurs when spread < gas cost at given capital size${RESET}`);
    console.log('');
    console.log(`${DIM}  Capital      Loss events    Total loss       Avg loss/trade${RESET}`);
    console.log(`${DIM}${'─'.repeat(62)}${RESET}`);
    for (const cap of [5000, 10000, 50000, 100000]) {
      const lossEvents = all.filter(o => {
        const p = cap * (parseFloat(o.spreadPct)/100) - GAS;
        return p < 0;
      });
      const totalLoss = lossEvents.reduce((sum, o) => {
        return sum + Math.abs(cap * (parseFloat(o.spreadPct)/100) - GAS);
      }, 0);
      const avgLoss = lossEvents.length > 0 ? totalLoss / lossEvents.length : 0;
      const lossColor = totalLoss > 0 ? RED : DIM;
      console.log(`  $${cap.toLocaleString().padEnd(10)} ${lossEvents.length.toString().padEnd(15)}${lossColor}$${totalLoss.toFixed(2).padEnd(17)}${RESET}$${avgLoss.toFixed(4)}`);
    }
    console.log('');

    // ─── NET P&L ─────────────────────────────────────────────────
    console.log(`${BOLD}NET P&L SUMMARY (realistic 46% capture, all events)${RESET}`);
    console.log(`${DIM}${'─'.repeat(62)}${RESET}`);
    console.log(`${DIM}  Capital      Gross profit     Total losses     NET P&L${RESET}`);
    console.log(`${DIM}${'─'.repeat(62)}${RESET}`);
    for (const cap of [5000, 10000, 50000, 100000]) {
      const gross = all.reduce((sum, o) => {
        const p = cap * (parseFloat(o.spreadPct)/100) - GAS;
        return p > 0 ? sum + (p * CAPTURE) : sum;
      }, 0);
      const losses = all.reduce((sum, o) => {
        const p = cap * (parseFloat(o.spreadPct)/100) - GAS;
        return p < 0 ? sum + Math.abs(p) : sum;
      }, 0);
      const net = gross - losses;
      const netColor = net >= 0 ? GREEN : RED;
      console.log(`  $${cap.toLocaleString().padEnd(10)} $${gross.toFixed(2).padEnd(16)} $${losses.toFixed(2).padEnd(16)}${netColor}$${net.toFixed(2)}${RESET}`);
    }
    console.log('');
